...
func TestClient_SetACL(t *testing.T) {
	...
	assert.NoError(t, client.PolicyFile().SetACL(context.Background(), expectedACL))
...
func TestClient_SetACL_HuJSON(t *testing.T) {
	...
	assert.NoError(t, client.PolicyFile().SetACL(context.Background(), string(huJSONACL)))
	assert.Equal(t, http.MethodPost, server.Method)
	assert.Equal(t, "/api/v2/tailnet/example.com/acl", server.Path)
	assert.Equal(t, "", server.Header.Get("If-Match"))
...
func TestClient_SetACLWithETag(t *testing.T) {
	...
	assert.NoError(t, client.PolicyFile().SetACL(context.Background(), expectedACL, tailscale.WithETag("test-etag")))
	assert.Equal(t, http.MethodPost, server.Method)
	assert.Equal(t, "/api/v2/tailnet/example.com/acl", server.Path)
...
func TestClient_ACL(t *testing.T) {
	...
	acl, err := client.PolicyFile().ACL(context.Background())
...
func TestClient_RawACL(t *testing.T) {
	...
	acl, err := client.PolicyFile().RawACL(context.Background())
	assert.NoError(t, err)
...
func TestClient_ValidateACL(t *testing.T) {
	...
	err := client.PolicyFile().ValidateACL(context.Background(), acl)
	assert.NoError(t, err)
...
func TestClient_ValidateACL_HuJSON(t *testing.T) {
	...
	err := client.PolicyFile().ValidateACL(context.Background(), string(huJSONACL))
	assert.NoError(t, err)
