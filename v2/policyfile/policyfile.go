...
type (
	...
	ACLSSH struct {
		...
		Source          []string           `json:"src,omitempty" hujson:"Src,omitempty"`
		Destination     []string           `json:"dst,omitempty" hujson:"Dst,omitempty"`
		CheckPeriod     tailscale.Duration `json:"checkPeriod,omitempty" hujson:"CheckPeriod,omitempty"`
...
func (c *Client) RawACL(ctx context.Context) (string, error) {
	...
	req, err := c.BuildRequest(ctx, http.MethodGet, fmt.Sprintf(uriFmt, c.Tailnet()), tailscale.RequestContentType("application/hujson"))
	if err != nil {
		return "", err
	}
...
func (c *Client) SetACL(ctx context.Context, acl any, opts ...SetACLOption) error {
	...
	reqOpts := []tailscale.RequestOption{
		tailscale.RequestHeaders(p.headers),
		tailscale.RequestBody(acl),
	}
	switch v := acl.(type) {
	case ACL:
	case string:
		reqOpts = append(reqOpts, tailscale.RequestContentType("application/hujson"))
	default:
...
func (c *Client) ValidateACL(ctx context.Context, acl any) error {
	...
	var response tailscale.APIError
