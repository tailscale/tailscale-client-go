...
func TestClient_CreateKey(t *testing.T) {
	...
	actual, err := client.Keys().CreateKey(context.Background(), capabilities)
...
func TestClient_CreateKeyWithExpirySeconds(t *testing.T) {
	...
	actual, err := client.Keys().CreateKey(context.Background(), capabilities, tailscale.WithKeyExpiry(1440*time.Second))
	assert.NoError(t, err)
	assert.EqualValues(t, expected, actual)
	assert.Equal(t, http.MethodPost, server.Method)
	assert.Equal(t, "/api/v2/tailnet/example.com/keys", server.Path)
...
func TestClient_CreateKeyWithDescription(t *testing.T) {
	...
	actual, err := client.Keys().CreateKey(context.Background(), capabilities, tailscale.WithKeyDescription("key description"))
	assert.NoError(t, err)
	assert.EqualValues(t, expected, actual)
	assert.Equal(t, http.MethodPost, server.Method)
	assert.Equal(t, "/api/v2/tailnet/example.com/keys", server.Path)
...
func TestClient_GetKey(t *testing.T) {
	...
	actual, err := client.Keys().GetKey(context.Background(), expected.ID)
	assert.NoError(t, err)
	assert.EqualValues(t, expected, actual)
...
func TestClient_Keys(t *testing.T) {
	...
	actual, err := client.Keys().Keys(context.Background())
	assert.NoError(t, err)
	assert.EqualValues(t, expected, actual)
	assert.Equal(t, http.MethodGet, server.Method)
	assert.Equal(t, "/api/v2/tailnet/example.com/keys", server.Path)
...
func TestClient_DeleteKey(t *testing.T) {
	...
	assert.NoError(t, client.Keys().DeleteKey(context.Background(), keyID))
...
func TestIsNotFound(t *testing.T) {
	...
	_, err := client.Keys().GetKey(context.Background(), "test")
