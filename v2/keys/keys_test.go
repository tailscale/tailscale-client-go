...
func TestClient_GetKey_IsNotFound(t *testing.T) {
	t.Parallel()
	...
	_, err := With(client).GetKey(context.Background(), "test")
...
func TestClient_CreateKey(t *testing.T) {
	...
	actual, err := With(client).CreateKey(context.Background(), capabilities)
...
func TestClient_CreateKeyWithExpirySeconds(t *testing.T) {
	...
	actual, err := With(client).CreateKey(context.Background(), capabilities, WithKeyExpiry(1440*time.Second))
	assert.NoError(t, err)
	assert.EqualValues(t, expected, actual)
	assert.Equal(t, http.MethodPost, server.Method)
	assert.Equal(t, "/api/v2/tailnet/example.com/keys", server.Path)
...
func TestClient_CreateKeyWithDescription(t *testing.T) {
	...
	actual, err := With(client).CreateKey(context.Background(), capabilities, WithKeyDescription("key description"))
	assert.NoError(t, err)
	assert.EqualValues(t, expected, actual)
	assert.Equal(t, http.MethodPost, server.Method)
	assert.Equal(t, "/api/v2/tailnet/example.com/keys", server.Path)
...
func TestClient_GetKey(t *testing.T) {
	...
	actual, err := With(client).GetKey(context.Background(), expected.ID)
	assert.NoError(t, err)
	assert.EqualValues(t, expected, actual)
...
func TestClient_Keys(t *testing.T) {
	...
	actual, err := With(client).Keys(context.Background())
	assert.NoError(t, err)
	assert.EqualValues(t, expected, actual)
	assert.Equal(t, http.MethodGet, server.Method)
	assert.Equal(t, "/api/v2/tailnet/example.com/keys", server.Path)
...
func TestClient_DeleteKey(t *testing.T) {
	...
	assert.NoError(t, With(client).DeleteKey(context.Background(), keyID))
